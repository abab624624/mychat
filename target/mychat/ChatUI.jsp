...
function winResize(){   
	...
	winResize();
	var receiver='${param.user2}';
	function addNotification()
	{
		
		$.ajax({
			url:"${ICHAT_PATH}/AddNotification",
			data:"sender="+'${username}'+"&receiver="+'${param.user2}',
			type:"POST",
...
$("#btn_send").click(function(){
	...
	$.ajax({
		url:"${ICHAT_PATH}/InsertContent",
		data:"touser="+'${param.user2}'+"&content="+$('#editArea').text() ,
		type:"POST",
		success:function(result){
			if(result.code==100)	{
				$("#editArea").empty();
	...
	function query_content(touser) {
		
		$.ajax({
			url:"${ICHAT_PATH}/QueryContentWithJson",
			data:"touser="+touser,
			type:"POST",
			success:function(result){
	...
	function query_content_first(touser) {
		
		
		$.ajax({
			url:"${ICHAT_PATH}/QueryContentWithJsonFirst",
			data:"touser="+touser,
			type:"POST",
			success:function(result){
...
/* 	var t=setTimeout("query_content('${param.user2}')",3000); */
	
	function build_content_table(result,ele){
		$(ele).empty();
		var contents = result.extend.LtlbList;
		...
		var fontdefault;
			...
			if(item.myread==0)read='未读';
			 ...
			 fontred=$("<font></font>").attr("color","red").append(read);
	    	  fontdefault=$("<font></font>").append(read);
			  if(item.username =='${username}'){
				  if(item.myread==0)
			 contentdiv.append(nicknamediv).append(fontred).append(divdirect);
				...
				if(item.myread==0)
				  contentdiv.append(nicknamediv).append(divdirect);
				
			    if(item.myread==1)  contentdiv.append(nicknamediv).append(divdirect);
			  }
...
/* 	var timer=self.setInterval("query_content('${param.user2}')",500); */
	
	$("#my_wechat_pic").click(function(){
	...
	$(function(){
		...
		$.ajax({
			url:"${ICHAT_PATH}/ClearNotification",
			data:"sender="+'${username}'+"&receiver="+'${param.user2}' ,
			type:"POST",
			success:function(result){
				if(result.code==100)	{
	...
	query_content_first('${param.user2}');
